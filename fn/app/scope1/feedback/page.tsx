          ...
          >
            <svg
              className={`w-8 h-8 transition-colors ${
                star <= (hover || value) ? 'text-indigo-600' : 'text-gray-300'
              }`}
...
export default function Scope1FeedbackPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      let email = ''
      const userStr = sessionStorage.getItem('scope1_user')
      if (userStr) {
        try {
      ...
      router.push('/scope1/feedback/thank-you')
    }
  }
        ...
        <div className="p-8">
          <div className="mb-2">
            <h3 className="text-sm font-semibold text-gray-800 flex items-center gap-2">
              <span className="w-1 h-1 bg-indigo-600 rounded-full inline-block" />
              How Was Your Experience With The Scope 1 Assessment?
            </h3>
            <StarRating
